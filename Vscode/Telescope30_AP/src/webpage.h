...
<body>
    ...
    <script>
        ...
        function InitWebSocket() {
            ...
            const ws_url = 'ws://192.168.4.1:81/'; // IP de l AP réelle 
        ...
        */
        ...
        function InitWebSocketCamera() {
            // Évite de multiplier les connexions si la fonction est rappelée
            if (websockCam && (websockCam.readyState === WebSocket.CONNECTING || websockCam.readyState === WebSocket.OPEN)) {
                return;
            }

            try {
              //websockCam = new WebSocket('ws://camera.local:82/');
                websockCam = new WebSocket('ws://192.168.4.10:82/'); // WebSocket unifié pour la caméra
                websockCam.binaryType = 'arraybuffer';

                websockCam.onopen = function () {
                ...
                websockCam.onmessage = function (event) {
                    // Vérifie si les données sont binaires (image) ou texte/JSON (commandes)
                    if (event.data instanceof ArrayBuffer) {
                        if (typeof handleVideoStream === 'function') {
                            handleVideoStream(event.data);
                        }
                    } else {
                        if (typeof handleCameraMessage === 'function') {
                            handleCameraMessage(event.data);
                        }
                    }
                };
                ...
                websockCam.onerror = function (error) {
                    // Affiche un simple avertissement au lieu d'une erreur critique
                    console.warn("⚠️ WebSocket Caméra indisponible (ESP32 hors tension ?)");
                };

                websockCam.onclose = function () {
                    console.log("🔄 WebSocket Caméra fermé.");
                    // Tente une reconnexion passive toutes les 5 secondes sans bloquer le reste de la page
                    if (!camReconnectTimer) {
                        camReconnectTimer = setTimeout(function () {
                            camReconnectTimer = null;
                            InitWebSocketCamera();
                        }, 5000);
                    }
                ...
                console.warn("Impossible d'initialiser le WebSocket Caméra :", e);
            }
        }
        ...
        function updateDOMElements(data) {
            ...
            for (const [id, value] of Object.entries(elementsToUpdate)) {
                ...
                if (el) {
                    ...
                    console.warn(`⚠️ ÉLÉMENT ABSENT DANS LE HTML : ID "${id}" introuvable.`);
                }

                if (data.temps_de_cycle !== undefined) {
                    addTcyclePoint(Number(data.temps_de_cycle));
                }
            }
        }
        ...
        function sendCommand(command) {
            ...
            //console.log("Position :", globalData.Position);
        ...
        // Synchroniser l'horloge de l'ESP32 avec l'horloge de la tablette.
        // Date.now() fournit le temps UTC en millisecondes, sans dépendre
        // du fuseau horaire configuré sur la tablette.

        /*
        function syncTimeToESP32() {

            // Remplace 192.168.1.50 par l'adresse IP de ton ESP32
            const esp32Ip = 'http://192.168.4.1';
            fetch(esp32Ip + '/settime?epoch=' + Date.now())
                .then(response => response.text())
                .then(text => console.log('⏱️ Heure ESP32 :', text))
                .catch(error => console.error('Erreur synchronisation heure:', error));
        }
        */
        ...
        function syncTimeToESP32() {
            ...
            // Heure UTC sous forme d'epoch en millisecondes
            const epoch = Date.now();

            // Décalage du navigateur par rapport à UTC.
            //
            // JavaScript donne :
            // France hiver : -60
            // France été   : -120
            //
            // On inverse le signe pour obtenir :
            // France hiver : +1
            // France été   : +2
            ...
            console.log('⏱️ Envoi heure UTC :', epoch);
            console.log('🌍 Décalage local :', offset);
            console.log('📡 URL :', url);

            fetch(url)
                .then(response => response.text())
                .then(text => {
                    console.log('⏱️ Réponse ESP32 :', text);
                })
                .catch(error => {
                    console.error('Erreur synchronisation heure:', error);
                });
        }

        // Synchronisation immédiate puis périodique.
        syncTimeToESP32();
        setInterval(syncTimeToESP32, 30000); // toutes les 5 minutes 300000
